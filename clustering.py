...
# Configuration
POSITIVE_DIR = r"C:\Users\visha\Desktop\crack_project\METU\Positive"
NEGATIVE_DIR = r"C:\Users\visha\Desktop\crack_project\METU\Negative"
RESULTS_DIR = "C:/Users/visha/Desktop/crack_project/results_clustering"
IMG_SIZE = (224, 224)
N_CLUSTERS = [2, 4, 6]
PCA_N_COMPONENTS = 50
...
def load_images():
    """Load all images from positive and negative directories."""
    pos_images = [os.path.join(POSITIVE_DIR, f) for f in os.listdir(POSITIVE_DIR) if f.endswith(('.jpg', '.png'))]
    neg_images = [os.path.join(NEGATIVE_DIR, f) for f in os.listdir(NEGATIVE_DIR) if f.endswith(('.jpg', '.png'))]
...
def evaluate_clusters(features, clusters, labels, method_name):
...
def plot_tsne(features, clusters, labels, method_name):
...
def plot_pca(features, clusters, labels, method_name):
...
def plot_cluster_histogram(clusters, method_name):
...
def plot_som(som, features, labels, method_name):
...
def main():
    ...
    image_paths, true_labels = load_images()
    ...
    for method_name, cluster_func in clustering_methods:
        ...
        for k in N_CLUSTERS:
            ...
            logger.info(f"{method_name} (k={k}) PCA metrics: {metrics_pca}")
            logger.info(f"{method_name} (k={k}) UMAP metrics: {metrics_umap}")

            # Visualize
            plot_tsne(pca_features, clusters_pca, true_labels, f"{method_name}_k{k}_pca")
            plot_tsne(umap_features, clusters_umap, true_labels, f"{method_name}_k{k}_umap")
            plot_pca(pca_features, clusters_pca, true_labels, f"{method_name}_k{k}_pca")
            plot_pca(umap_features, clusters_umap, true_labels, f"{method_name}_k{k}_umap")
            plot_cluster_histogram(clusters_pca, f"{method_name}_k{k}_pca")
            plot_cluster_histogram(clusters_umap, f"{method_name}_k{k}_umap")

    # SOM clustering
    logger.info("Running SOM clustering...")
    ...
    metrics_som = evaluate_clusters(features, clusters_som, true_labels, "som")
    logger.info(f"SOM metrics: {metrics_som}")
    plot_som(som, features, true_labels, "som")
    plot_cluster_histogram(clusters_som, "som")
