...
class NestedAutoencoder:
    ...
    def train(self, train_images, iteration, batch_size=32, epochs=50, validation_split=0.2):
        ...
        Args:
            ...
            iteration: Iteration number (set to 1)
    ...
    def save_results_summary(self, training_time, evaluation_metrics, iteration):
        """Save a summary of results for the iteration"""
        iter_dir = os.path.join(self.results_dir, f'iteration_{iteration}')
...
def main():
    ...
    # Train and evaluate for a single iteration
    print("\nStarting Single Iteration")
    start_time = time.time()

    # Train model
    model.train(train_images, iteration=1, batch_size=32, epochs=50)

    # Evaluate model
    evaluation_metrics = model.evaluate(test_images, test_labels, test_filenames, iteration=1)

    # Save summary
    training_time = time.time() - start_time
    model.save_results_summary(training_time, evaluation_metrics, iteration=1)

    print(f"\nSingle iteration completed in {training_time:.2f} seconds")
